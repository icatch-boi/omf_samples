...
static remotePushErrno_e remotePushMsgProcess(remotePushMsgBody_t *msgBody)
{
	...
	switch(msgBody->baseParam.type) {
		...
		case MSG_EVT_OTA_INSTALL_ERR:
			...
			break;
		default:
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s:%d]error:this kind message is not supported",__FUNCTION__ ,__LINE__);
			return REMOTE_PUSH_ERRNO_PARAM;
	}
	
	/* pushing message */
	remotePushModulePushMsg(&msgParam);
	return REMOTE_PUSH_ERRNO_SUCCESS;
}
...
static remotePushErrno_e remotePushFileProcess(remotePushFileBody_t *fileBody)
{
	...
	return remotePushModulePushFile(&genericParam);
}
...
void remotePushModuleInit()
{
	...
	remotePushModuleMsgTaskCreate();
	remotePushModuleFileTaskCreate();
}
