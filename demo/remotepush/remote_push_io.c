...
static tinyaiotOauth2Client_t gtinyaiotOauth2Client = {0};
...
static tinyaiotOauth2Client_t* tinyaiotOauth2ClientGet()
{
	UINT32 idLength = 128;
	UINT32 secretLength = 128;
	UINT32 uriLength = 128;

	if(NULL==gtinyaiotOauth2Client.id) {
		gtinyaiotOauth2Client.id = (char*)malloc(idLength);
		if(NULL==gtinyaiotOauth2Client.id) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] no memory", __FUNCTION__,__LINE__);
			return NULL;
		}
		memset(gtinyaiotOauth2Client.id, 0, idLength);
	}

	if(NULL==gtinyaiotOauth2Client.secret) {
		gtinyaiotOauth2Client.secret = (char*)malloc(secretLength);
		if(NULL==gtinyaiotOauth2Client.secret) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] no memory", __FUNCTION__,__LINE__);
			return NULL;
		}
		memset(gtinyaiotOauth2Client.secret, 0, secretLength);
	}

	if(NULL==gtinyaiotOauth2Client.redirect_uri) {
		gtinyaiotOauth2Client.redirect_uri = (char*)malloc(uriLength);
		if(NULL==gtinyaiotOauth2Client.redirect_uri) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] no memory", __FUNCTION__,__LINE__);
			return NULL;
		}
		memset(gtinyaiotOauth2Client.redirect_uri, 0, uriLength);
	}

	/* sync from flash */
	if(NULL != gtinyaiotOauth2Client.id && 0 == strlen(gtinyaiotOauth2Client.id)) {
		int iRet = iniGet(INI_SECTION_SYS, INI_KEY_OAUTH2_CLIENT_ID , "", gtinyaiotOauth2Client.id, idLength, INI_CFG_FILENAME);
		if(0 == iRet) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] read(%s) from (%s) error", __FUNCTION__,__LINE__, INI_KEY_OAUTH2_CLIENT_ID, INI_CFG_FILENAME);
			return NULL;
		}
	}

	if(NULL != gtinyaiotOauth2Client.secret && 0 == strlen(gtinyaiotOauth2Client.secret)) {
		int iRet = iniGet(INI_SECTION_SYS, INI_KEY_OAUTH2_CLIENT_SECRET , "", gtinyaiotOauth2Client.secret, secretLength, INI_CFG_FILENAME);
		if(0 == iRet) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] read(%s) from (%s) error", __FUNCTION__,__LINE__, INI_KEY_OAUTH2_CLIENT_SECRET, INI_CFG_FILENAME);
			return NULL;
		}
	}

	if(NULL != gtinyaiotOauth2Client.redirect_uri && 0 == strlen(gtinyaiotOauth2Client.redirect_uri)) {
		int iRet = iniGet(INI_SECTION_SYS, INI_KEY_OAUTH2_CLIENT_URI , "", gtinyaiotOauth2Client.redirect_uri, uriLength, INI_CFG_FILENAME);
		if(0 == iRet) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] read(%s) from (%s) error", __FUNCTION__,__LINE__, INI_KEY_OAUTH2_CLIENT_URI, INI_CFG_FILENAME);
			return NULL;
		}
	}
	dbgLogPrint(REMOTE_PUSH_LVL_INFO, "[%s-%d] gtinyaiotOauth2Client.id(%s)", __FUNCTION__,__LINE__, gtinyaiotOauth2Client.id);
	dbgLogPrint(REMOTE_PUSH_LVL_INFO, "[%s-%d] gtinyaiotOauth2Client.secret(%s)", __FUNCTION__,__LINE__, gtinyaiotOauth2Client.secret);
	dbgLogPrint(REMOTE_PUSH_LVL_INFO, "[%s-%d] gtinyaiotOauth2Client.redirect_uri(%s)", __FUNCTION__,__LINE__, gtinyaiotOauth2Client.redirect_uri);
	return &gtinyaiotOauth2Client;
}
...
{
	int iRet = TINYAIOT_SUCCEEDED;
	g_tiny_aiot_cnt++;
	dbgLogPrint(REMOTE_PUSH_LVL_DEBUG, "[%s-%d]g_tiny_aiot_cnt(%d)", __FUNCTION__,__LINE__, g_tiny_aiot_cnt);
	if(1 == g_tiny_aiot_cnt) {
		iRet = tinyAIoT_Initialize2(TINYAIOT_TRANS_TYPE_ID_HTTP,tinyaiotSaveDevInfoCB,tinyaiotReadDevInfoCB);
		if(iRet) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] ERRORNO(%d)", __FUNCTION__,__LINE__,iRet);
			g_tiny_aiot_cnt--;
			return REMOTE_PUSH_ERRNO_OTHERS;
		}
		iRet = tinyAIoT_SystemSet(TINYAIOT_PARAM_ID_OAUTH2_CLIENTS, (void*)tinyaiotOauth2ClientGet());
		if (iRet) {
			dbgLogPrint(REMOTE_PUSH_LVL_ERROR, "[%s-%d] ERRORNO(%d)", __FUNCTION__,__LINE__,iRet);
			g_tiny_aiot_cnt--;
			return REMOTE_PUSH_ERRNO_OTHERS;
	...
	char *pServURL = NULL;
	remotePushErrno_e eRet = remotePushModuleServUrlGet(&pServURL);
	if(REMOTE_PUSH_ERRNO_SUCCESS != eRet) {
	...
	iRet = tinyAIoT_SystemSet(TINYAIOT_PARAM_ID_DEV_SERVER_ADDR, pServURL);
	if(TINYAIOT_SUCCEEDED != iRet) {
...
remotePushErrno_e remotePushModuleSendMsg(remotePushGenericMsgParam_t *genericParam)
{
	...
	if(REMOTE_PUSH_ERRNO_SUCCESS != remotePushModuleIOInit(tinyaiotSaveDevInfoCB, tinyaiotReadDevInfoCB)) {
...
remotePushErrno_e remotePushModuleSendFile(remotePushGenericFileParam_t *genericParam)
{
	...
	while(retry--) {
		...
		if(TINYAIOT_CALL_REPEATED == iRet) {
			break;
		}
		usleep(50*1000);
	}
